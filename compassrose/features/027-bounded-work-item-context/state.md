...
Context is now declared and measured. `promptBuilding.ts` is a renderer with no authority to include
anything the manifest does not name, and the budget is checked at planning time -- so an oversized
task costs one planning call instead of an implementation call that half-writes something.

What remains is the other direction: the implementer does not yet report what it read beyond its
manifest, so the exploration merge, which exists and is tested, has nothing feeding it.
...
Have the implementer report what it read beyond its manifest, and enforce the declared exploration cap at the adapter layer.
