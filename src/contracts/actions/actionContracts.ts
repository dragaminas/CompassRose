import type { PlannedFeatureDocs } from "../planner/plannerContracts.js";
import type { DiagnosticAutocorrectionDecision } from "../runtime/diagnosticAutocorrection.js";
import type { ImplementationAttempt } from "../runtime/attempts.js";
import type { RecoveryLesson } from "../runtime/taskInterfaceAnalysis.js";
import type { FeatureStateSnapshot } from "../state/featureStateSnapshot.js";
import type { ProjectStateSnapshot } from "../state/projectStateSnapshot.js";
...
import type {
  ...
  StateCorrectionTask,
} from "../task/taskContracts.js";

/**
 * Action contracts for the runtime loop.
 *
 * `ActionType` says what the runtime can do. The context interfaces describe
 * the minimum data each action needs in order to execute.
 */
export enum ActionType {
  ...
  // Formalizes a found feature request into the canonical feature documents and feature outline.
  PLAN_FEATURE = "plan_feature",
  // Creates one bounded implementation task from the feature outline.
  PLAN_TASK = "plan_task",
  // Refines the current task into the next bounded execution iteration.
  PLAN_SUBTASK = "plan_subtask",
  // Sends the current task iteration for implementation.
  IMPLEMENT_SUBTASK = "implement_subtask",
  // Runs the configured quality gates before review.
  RUN_QUALITY_GATES = "run_quality_gates",
  // Sends the current task iteration for review.
...
export interface ActionContextBase {
  ...
  readonly projectStateSnapshot?: ProjectStateSnapshot | undefined;
