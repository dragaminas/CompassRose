...
- `implementation.implementation_notes` from the attempt artifact, when available
- Implementation context artifacts that record the exact prompt and runtime snapshot sent to the implementer, supplied as separate file paths alongside the task (not as a field inside `implementation`) and named explicitly in the prompt when present
...
- Inspect the implementation context artifacts, when supplied, before rejecting the attempt so context restrictions can be diagnosed explicitly; their absence alone is not a defect to report.
