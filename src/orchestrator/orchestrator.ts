...
export class CompassRoseOrchestrator {
   ...
   */
  /**
   * Records who decided what, on the specification itself (024-specification-flow).
   *
   * The competency profile is never written anywhere -- it belongs to the person in the session,
   * and persisting it would impose one author's competencies on everyone after them. This is the
   * other half of that rule: what *is* written is a fact about the document. "The human chose the
   * second option" and "nobody was asked and the agent chose the second option" produce identical
   * specification text and are not the same fact about it, and only one of them is safe for a
   * later reader to build on without checking.
   *
   * Appended rather than woven into each section: a section-level marker would have to be
   * regenerated by whatever rewrites that section, and a provenance claim that silently survives a
   * rewrite is worse than none.
   */
  /**
   * Adds a dimension the agent proposed and a human accepted.
   *
   * Enters as `uncovered` with the human's own reason attached, never as covered: proposing a
   * dimension and covering it are two different acts, and only the second says the specification
   * actually addresses it.
   */
  proposeDimension(name: string, why: string, by: string): void {
    const trimmed = name.trim();
    const dimensions = this.readDimensions();
    if (dimensions.some((dimension) => dimension.name.toLowerCase() === trimmed.toLowerCase())) {
      return;
    }

    const path = join(this.repositoryRoot, buildDimensionsPath(this.compassRoseRoot));
    writeText(path, renderDimensionsDocument([
      ...dimensions,
      {
        name: trimmed,
        state: 'uncovered',
        ...
        decisions: [
          {
            state: 'uncovered',
            reason: `proposed while specifying: ${why.trim()}`,
            by,
  ...
  recordSpecificationProvenance(
    ...
    const lines = [
      '## Provenance',
      '',
      `Specified on ${new Date().toISOString().slice(0, 10)}. Who owned each axis in that session:`,
      '',
      ...COMPETENCY_AXES.map((axis) => `- ${axis}: ${profile[axis]}`),
      '',
      ...(decisions.length > 0
        ? [
            'Decisions taken while specifying it:',
            '',
            ...decisions.map((decision) =>
              `- ${decision.question} -> ${decision.chosen} (${decision.axis}, decided by the ${decision.decided_by})`,
            ),
          ]
    ...
    const markdown = readUtf8(owner.definitionPath);
    writeText(
      owner.definitionPath,
      markdown.includes('## Provenance')
        ? replaceSection(markdown, 'Provenance', lines.slice(2).join('\n').trimEnd())
        : `${ensureTrailingNewline(markdown)}\n${lines.join('\n')}`,
    );

    this.commitDirtyWorktreeIfConfigured(`proto: record specification provenance for ${id}`);
  }
   ...
   */
  /**
   * One turn of the specification conversation.
   *
   * `profile` is what makes the competency declaration mean something. Until it reached this
   * prompt, declaring who owned which axis changed nothing about how the agent behaved: it was a
   * fact the session held and nobody acted on. Now a human-owned axis obliges the agent to surface
   * a decision instead of taking one, and an agent-owned axis obliges it to take the decision and
   * say why rather than asking.
   */
  runBrainstormTurn(
    transcript: readonly BrainstormTurnRecord[],
    userMessage: string,
    profile: SessionCompetencyProfile = DEFAULT_COMPETENCY_PROFILE,
  ): BrainstormTurnOutput {
    ...
    const prompt = [
      ...
      ...(siblingFeatures.length > 0
        ...
        : ['- none']),
      '',
      'Who decides what, in this session:',
      ...describeProfileForPrompt(profile),
      '',
      'Dimensions of this application, and whether the specification has covered them:',
      ...this.describeDimensionsForPrompt(),
      '',
      'Conversation so far:',
      transcriptText,
      '',
      ...
      '- When a real choice arises on an axis the human owns, put it in `decision` with two to four concrete options, and say what each one commits the project to rather than restating its label. Do not also argue for one in `reply`; recommending is what `recommended_index` is for.',
      '- Leave `decision` null when there is no genuine choice, which is most turns. Raising one every turn is how a conversation stops distinguishing decisions from details.',
      '- Never put a decision on an axis you own. On those, choose, and say in `reply` why you chose it.',
      '- Set `proposed_dimension` when the conversation reveals a dimension of this application that the list above does not cover and that this project in particular needs. Never a generic checklist item, and never one already listed. Leave it null otherwise.',
      '- Do not modify files.',
      '',
  ...
  /**
   * The dimensions checklist as the conversation sees it.
   *
   * Shown so the agent can notice a gap, never so it can close one: covering a dimension is a
   * human decision recorded through `decideDimension`, exactly like discarding one.
   */
  private describeDimensionsForPrompt(): string[] {
    const dimensions = this.readDimensions();
    if (dimensions.length === 0) {
      return ['- none declared yet'];
    }

    return dimensions.map((dimension) => `- ${dimension.name}: ${dimension.state}`);
  }
